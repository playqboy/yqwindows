...
var Game = (function() {
  var STATES = { LOADING: 'loading', PLAYING: 'playing', READY: 'ready', ENDED: 'ended', RESETTING: 'resetting' };
  var GAME_OVER_MESSAGES = [
    function(s) { return "Congrats, you just wasted " + s + " second" + (s === 1 ? "" : "s"); },
    function() { return "You're literally playing a game on yq's site instead of being productive"; },
    function() { return "Bro???"; },
    function() { return "Dude, is this why you came here?"; },
    function() { return "Your boss is gonna love this on your timesheet"; },
    function() { return "Imagine if you spent this time learning to code"; },
    function() { return "yq built this site faster than you stacked those blocks"; },
    function() { return "This is going on your permanent record"; },
    function() { return "Alt+F4 might be a better use of your time"; },
    function() { return "Your LinkedIn says 'hard worker' btw"; },
    function() { return "Somewhere, a recruiter just closed your tab"; }
  ];
  function Game() {
    this.STATES = STATES;
    this.blocks = [];
  ...
  Game.prototype.restartGame = function() {
    ...
    for (var i = 0; i < oldBlocks.length; i++) {
      TweenLite.to(oldBlocks[i].scale, removeSpeed, { x: 0, y: 0, z: 0, delay: (oldBlocks.length - i) * delayAmount, ease: Power1.easeIn, onComplete: (function(block) { return function() { self.placedBlocks.remove(block); }; })(oldBlocks[i]) });
  ...
  Game.prototype.placeBlock = function() {
    ...
    if (newBlocks.chopped) {
      ...
      var positionParams = { y: '-=30', ease: Power1.easeIn, onComplete: (function(chopped, choppedBlocks) { return function() { choppedBlocks.remove(chopped); }; })(newBlocks.chopped, this.choppedBlocks) };
  ...
  Game.prototype.endGame = function() {
    var elapsed = Math.round((Date.now() - this.gameStartTime) / 1000);
    var msgFn = GAME_OVER_MESSAGES[Math.floor(Math.random() * GAME_OVER_MESSAGES.length)];
    document.getElementById('game-over-msg').textContent = msgFn(elapsed);
...
const GameWindow: React.FC<GameWindowProps> = ({ onClose, zIndex, onFocus }) => {
